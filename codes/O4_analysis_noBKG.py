...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O4_noBKG_rate_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O4_rate_noBKG.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O4_rate_noBKG.yaml")
...
if fit:
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('../graph/O4_noBKG_loss_rate.pdf')
...
if fit:
    ...
    plt.tight_layout()
    plt.savefig('../graph/O4_noBKG_Coverage_rate.pdf')
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O4_noBKG_drate_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O4_noBKG_drate.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O4_noBKG_drate.yaml")
    email('Termino el entramiento del drate para O4_noBKG')
else:
    ckpt_path = swyft.best_from_yaml("./logs/O4_noBKG_drate.yaml")
...
if fit:
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('../graph/O4_noBKG_loss_drate.pdf')
...
if fit:
    ...
    for i in range(3):
        swyft.plot_zz(coverage_samples, "pars_norm[%i]"%i, ax = axes[i])
    plt.tight_layout()
    plt.savefig('../graph/O4_noBKG_Coverage_drate.pdf')
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O4_noBKG_s1s2_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O4_noBKG_s1s2.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O4_noBKG_s1s2.yaml")
else:
    ckpt_path = swyft.best_from_yaml("./logs/O4_noBKG_s1s2.yaml")
...
if fit:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('../graph/O4_noBKG_loss_s1s2_temp.pdf')
...
if fit:
    ...
    for i in range(3):
        swyft.plot_zz(coverage_samples, "pars_norm[%i]"%i, ax = axes[i])
    plt.tight_layout()
    plt.savefig('../graph/O4_noBKG_Coverage_s1s2.pdf')
...
for i, theta in enumerate([3,4,0]):
    pars_slices, rate_slices, diff_rate_slices, s1s2_slices = read_slice(['../data/andresData/O4-fulldata/O4/theta-' + thetas[theta] + '/O4-slices01-' + thetas[theta] + '/'])
...
plt.savefig('../graph/O4_graph/O4_contours_all_int_prob_sup_COMB_noBKG.pdf')
# -
