...
x_s1s2 = s1s2_trainset.reshape((8174,9409)) # Observable. Input data. I am cutting a bit the images to have 64x64

x_s1s2 = np.hstack((x_s1s2,diff_rate_trainset))
...
ax[0,0].scatter(x_norm_s1s2[:,12], x_norm_s1s2[:,120])
ax[0,0].scatter(x_norm_test_s1s2[:,12],x_norm_test_s1s2[:,120])
ax[0,0].set_xlabel('# Events')
...
class Network(swyft.SwyftModule):
    def __init__(self, lr = 1e-3, gamma = 1.):
        ...
        marginals = ((0, 1), (0, 2), (1, 2))
        self.logratios1 = swyft.LogRatioEstimator_1dim(num_features = 15, num_params = 3, varnames = 'pars_norm')
        self.logratios2 = swyft.LogRatioEstimator_Ndim(num_features = 15, marginals = marginals, varnames = 'pars_norm')
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O1_s1s2_flat_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
# +
x_norm_test_s1s2 = s1s2_testset.reshape(len(s1s2_testset), 9409) # Observable. Input data. I am cutting a bit the images to have 96x96
x_norm_test_s1s2 = np.hstack((x_norm_test_s1s2, diff_rate_testset))
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O1_s1s2_flat.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_flat.yaml")
else:
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_flat.yaml")
...
if fit:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('../graph/O1_loss_s1s2_flat.pdf')
...
x_norm_s1s2 = s1s2_testset.reshape(len(s1s2_testset), 9409) # Observable. Input data. I am cutting a bit the images to have 96x96
x_norm_s1s2 = np.hstack((x_norm_s1s2, diff_rate_testset))

x_norm_s1s2 = x_norm_s1s2 / x_max_s1s2 # Observable. Input data. I am cutting a bit the images to have 96x96
# -
...
plt.imshow(x_obs[:9409].reshape(97,97).T, origin = 'lower')
