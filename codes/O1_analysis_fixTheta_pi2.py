...
def read_slice(datFolder):
    ...
    print('We have ' + str(nobs_slices) + ' observations before theta cut...')
    ...
    indices = np.where( (pars_slices[:,2] < 1.65) & (pars_slices[:,2] > .7))[0]
    pars_slices      = pars_slices[indices]
    rate_slices      = rate_slices[indices]
    rate_raw_slices  = rate_raw_slices[indices]
    s1s2_slices      = s1s2_slices[indices]
    diff_rate_slices = diff_rate_slices[indices]
    nobs_slices = len(indices)
...
indices = np.where( (pars[:,2] < 1.65) & (pars[:,2] > .7))[0]
pars = pars[indices]
rate = rate[indices]
rate_raw = rate_raw[indices]
s1s2 = s1s2[indices]
diff_rate = diff_rate[indices]
nobs = len(indices)
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O1_rate_fixTheta>0.7_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O1_rate_fixTheta>0.7.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O1_rate_fixTheta>0.7.yaml")
    #email('Termino de entrenar rate O1 con fix theta > 0.7')
    
else:
    ckpt_path = swyft.best_from_yaml("./logs/O1_rate_fixTheta>0.7.yaml")

# ---------------------------------------------- 
# It converges to val_loss = - at epoch ~
# ---------------------------------------------- 
...
# ---------------------------------------------- 
# It converges to val_loss ~ -0.96 in testset
# ---------------------------------------------- 
# -
...
if fit:
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('../graph/O1_loss_rate_fixTheta>0.7.pdf')
...
pars_slices, rate_slices, diff_rate_slices, s1s2_slices = read_slice(['../data/andresData/O1-slices-5vecescadatheta/theta-pluspidiv2/SI-slices01-pluspidiv2/'])
...
for folder in folders:
    ...
    if (os.path.exists(folder + 'cross_sec_sigmas_rate_thetaFix>0.7.txt') & 
        os.path.exists(folder + 'cross_sec_int_prob_rate_thetaFix>0.7.txt') &
        os.path.exists(folder + 'cross_sec_int_prob_sup_rate_thetaFix>0.7.txt') &
        os.path.exists(folder + 'masses_int_prob_sup_rate_thetaFix>0.7.txt')
       ) == False or force == True:
        # Let's normalize testset between 0 and 1
        ...
        np.savetxt(folder + 'cross_sec_sigmas_rate_thetaFix>0.7.txt', cross_sec_sigmas)
        np.savetxt(folder + 'cross_sec_int_prob_rate_thetaFix>0.7.txt', cross_sec_int_prob)
        np.savetxt(folder + 'cross_sec_int_prob_sup_rate_thetaFix>0.7.txt', cross_sec_int_prob_sup)
        np.savetxt(folder + 'masses_int_prob_sup_rate_thetaFix>0.7.txt', masses_int_prob_sup)
    else:
        print('pre-computed')
                
        cross_sec_sigmas = np.loadtxt(folder + 'cross_sec_sigmas_rate_thetaFix>0.7.txt')
        cross_sec_int_prob = np.loadtxt(folder + 'cross_sec_int_prob_rate_thetaFix>0.7.txt')
        cross_sec_int_prob_sup = np.loadtxt(folder + 'cross_sec_int_prob_sup_rate_thetaFix>0.7.txt')
        masses_int_prob_sup = np.loadtxt(folder + 'masses_int_prob_sup_rate_thetaFix>0.7.txt')
...
if len(cross_sec_int_prob_full) > 1:
    cross_sec_int_prob_pi_2        = np.mean(np.asarray(cross_sec_int_prob_full), axis = 0)
    cross_sec_int_prob_sup_pi_2    = np.mean(np.asarray(cross_sec_int_prob_sup_full), axis = 0)
    cross_sec_int_prob_sup_pi_2_sd = np.std(np.asarray(cross_sec_int_prob_sup_full), axis = 0)
    cross_sec_sigmas               = np.mean(np.asarray(cross_sec_sigmas_full), axis = 0)
    masses_int_prob_sup_pi_2       = np.mean(np.asarray(masses_int_prob_sup_full), axis = 0)
    masses_int_prob_sup_pi_2_sd    = np.std(np.asarray(masses_int_prob_sup_full), axis = 0)
else:
    cross_sec_int_prob_pi_2     = cross_sec_int_prob
    cross_sec_int_prob_sup_pi_2 = cross_sec_int_prob_sup
    masses_int_prob_sup_pi_2    = masses_int_prob_sup
...
# #%plt.savefig('../graph/O1_int_prob_distribution_rate_thetaFix>0.7.pdf')
...
# #%plt.savefig('../graph/O1_contours_rate_int_prob_thetaFix>0.7.pdf')
...
# #%plt.savefig('../graph/O1_contours_rate_int_prob_sup_thetaFix>0.7.pdf')
# -
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O1_s1s2_fixTheta>0.7_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O1_s1s2_fixTheta>0.7.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_fixTheta>0.7.yaml")
else:
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_fixTheta>0.7.yaml")
...
# ---------------------------------------
# Min val loss value at 7 epochs. -1.46 @ testset
# ---------------------------------------
...
if fit:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('../graph/O1_loss_s1s2_fixTheta>0.7.pdf')
...
for folder in folders:
    ...
    if (os.path.exists(folder + 'cross_sec_sigmas_s1s2_thetaFix>0.7.txt') & 
        os.path.exists(folder + 'cross_sec_int_prob_s1s2_thetaFix>0.7.txt') &
        os.path.exists(folder + 'cross_sec_int_prob_sup_s1s2_thetaFix>0.7.txt') &
        os.path.exists(folder + 'masses_int_prob_sup_s1s2_thetaFix>0.7.txt')
       ) == False or force == True:
        # Let's normalize testset between 0 and 1
        ...
        np.savetxt(folder + 'cross_sec_sigmas_s1s2_thetaFix>0.7.txt', cross_sec_sigmas)
        np.savetxt(folder + 'cross_sec_int_prob_s1s2_thetaFix>0.7.txt', cross_sec_int_prob)
        np.savetxt(folder + 'cross_sec_int_prob_sup_s1s2_thetaFix>0.7.txt', cross_sec_int_prob_sup)
        np.savetxt(folder + 'masses_int_prob_sup_s1s2_thetaFix>0.7.txt', masses_int_prob_sup)
    else:
        print('pre-computed')
        cross_sec_sigmas       = np.loadtxt(folder + 'cross_sec_sigmas_s1s2_thetaFix>0.7.txt')
        cross_sec_int_prob     = np.loadtxt(folder + 'cross_sec_int_prob_s1s2_thetaFix>0.7.txt')
        cross_sec_int_prob_sup = np.loadtxt(folder + 'cross_sec_int_prob_sup_s1s2_thetaFix>0.7.txt')
        masses_int_prob_sup    = np.loadtxt(folder + 'masses_int_prob_sup_s1s2_thetaFix>0.7.txt')
...
# #%plt.savefig('../graph/O1_int_prob_distribution_s1s2_fixTheta>0.7.pdf')
...
#plt.savefig('../graph/O1_contours_s1s2_int_prob_fixTheta>0.7.pdf')
...
#plt.savefig('../graph/O1_contours_s1s2_int_prob_sup_fixTheta>0.7.pdf')
# +
levels = [0.67, 0.76, 0.84, 0.9, 1]
...
ax[0].fill_between(masses, s1s2_current_pi2[2,:], 1e-43, color = 'black', alpha = 0.2, label = 'Excluded')
...
#plt.savefig('../graph/O1_contours_all_int_prob_sup_fixTheta>0.7.pdf')
# -
