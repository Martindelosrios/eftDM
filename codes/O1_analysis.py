...
# Profile Likelihoods (m4, Umu)
#{{{

labels = ['ESS + DARWIN', 'ESS', 'DARWIN']
colors = (c_color, ss_color, dd_color, c_color, ss_color, dd_color)
hatches = [[''],['//'], ['||']]

def prof_likelihood(ax, whichx = 'm4', whichy = 'Umu', exp_labels = True, plot_name = None, bins = [45,45,15,45,45,15], xname = 10**(-1.4), yname = 10**(-1.5), chi2_crit = 6.18, loc_labels = 'upper left'):
    if whichx == 'm4':
        ix = 0
        xpoint = m4
    elif whichx == 'Ue':
    ...
    if exp_labels:    
        ...
        for i in range(len(labels)):
            if globals()['f' + str(i)]:
                custom_lines.append( Line2D([0],[0], marker = '', linewidth = 3, linestyle = 'solid', color = colors[i], 
                    label = labels[i] ))
        ax.legend(handles = custom_lines, loc = loc_labels)
    
    
    ax.set_xlim(0.9e-3, 8e-2)
    ax.set_ylim(0.9e-4, 8e-2)
    ax.set_xlabel(parameters[ix])
    ax.set_ylabel(parameters[iy])
    ax.set_xscale('log')
    ax.set_yscale('log')
    #ax.set_xticks([1e-3, 1e-2, 1e-1], ['$10^{-3}$','$10^{-2}$','$10^{-1}$'])

    
    for i in range(len(data)):
         
        val, x, y,_ = stats.binned_statistic_2d(data[i][:,ix], data[i][:,iy], _2loglik[i][:], 'min', bins = [np.logspace(-3.5, -0.5, bins[i]), np.logspace(-4.5,-0.7, bins[i])])
        ...
        # ------------------------ COMBINED RESULTS SIGMA = 0.1 -------------------------------------------------
        if (i == 0) & f0:
            cs = ax.contourf(x_centers, y_centers, val.T, levels=[0, chi2_crit], colors = c_color, alpha = 0.6, zorder = 1)
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = c_color, linewidths = 2, zorder = 4)
        # ------------------------ COMBINED RESULTS SIGMA = 0.4 -------------------------------------------------
        elif (i == 3) & f3:
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = c_color, linestyles = ['--'], linewidths = 2, zorder = 1)
        # ------------------------ SS/DD RESULTS SIGMA = 0.1 -------------------------------------------------
        elif (i == 1) & f1:
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = colors[i], linewidths = 2, zorder = 3)
            cs = ax.contourf(x_centers, y_centers, val.T, levels=[0, chi2_crit], colors = 'none', hatches = hatches[i], zorder = 1)
            for ics, collection in enumerate(cs.collections):
                collection.set_edgecolor(mpl.colors.to_rgb(colors[i]))
            #for collection in cs.collections:
            #    collection.set_linewidth(0.)
        elif (i == 2) & f2:
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = colors[i], linewidths = 2, zorder = 3)
            cs = ax.contourf(x_centers, y_centers, val.T, levels=[-1, chi2_crit], colors = 'none', hatches = hatches[i], zorder = 1)
            for ics, collection in enumerate(cs.collections):
                collection.set_edgecolor(mpl.colors.to_rgb(colors[i]))
            #for collection in cs.collections:
            #    collection.set_linewidth(0.)
        # ------------------------ SS/DD RESULTS SIGMA = 0.4 -------------------------------------------------
        elif (i == 4) & f4:
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = colors[i], linestyles = ['--'], linewidths = 2, zorder = 1)
        elif (i == 5) & f5:
            ax.contour(x_centers, y_centers, val.T, levels=[chi2_crit], colors = colors[i], linestyles = ['--'], linewidths = 2, zorder = 1)
    ...
    if (whichx == 'm4') & (whichy == 'Umu'):
        ax.contourf(m4_grid, Umu_grid, CohAr, levels =[0,chi2_crit,100], colors = ['white','black','red'], alpha = 0.3, zorder = 2)
        ax.contourf(m4_grid, Umu_grid, CohCsI, levels=[0,chi2_crit,100], colors = ['white','black','red'], alpha = 0.3, zorder = 2)
    elif (whichx == 'm4') & (whichy == 'Ue'):
        ax.contourf(m4_grid, Umu_grid, CohAr_e, levels =[0,chi2_crit,100], colors = ['white','black','red'], alpha = 0.3, zorder = 2)
        ax.contourf(m4_grid, Umu_grid, CohCsI_e, levels=[0,chi2_crit,100], colors = ['white','black','red'], alpha = 0.3, zorder = 2)
    else:
        ax.contourf(m4_grid, Umu_grid, np.ones((100,100)), levels=[0,chi2_crit], colors = ['white','white'], alpha = 0.5, zorder = 2)
    
    # ------------------------ ARGUELLES CONSTRAINTS -------------------------------------------------
    if (whichx == 'Ue'): ax.axvline(x = U_e4_max, linestyle = '--')
    if (whichy == 'Ue'): ax.axhline(y = U_e4_max, linestyle = '--')
    if (whichx == 'Umu'): ax.axvline(x = U_mu4_max, linestyle = '--')
    if (whichy == 'Umu'): ax.axhline(y = U_mu4_max, linestyle = '--')
    if (whichx == 'Utau'): ax.axvline(x = U_tau4_max, linestyle = '--')
    if (whichy == 'Utau'): ax.axhline(y = U_tau4_max, linestyle = '--')
    # ------------------------ MAIN BPs         ------------------------------------------------
    if plot_name is not None: ax.text(xname, yname, plot_name, fontsize = 14)
    if (xpoint > 1e-4) & (ypoint > 1e-4):
        ax.scatter(xpoint, ypoint, marker='*',color='black', s = 220, zorder = 7)
        ax.scatter(xpoint, ypoint, marker='*',color='yellow', s = 40, zorder = 7)
    elif (xpoint > 1e-4) & (ypoint < 1e-4):
        ax.scatter(xpoint, 10**(-3.82), marker='v',color='black', s = 70, linewidths = 4, zorder = 7)
        ax.scatter(xpoint, 10**(-3.82), marker='v',color='yellow', s = 20, linewidths = 4, zorder = 7)
    elif (xpoint < 1e-4) & (ypoint > 1e-4):
        ax.scatter(10**(-3.82), ypoint, marker='<',color='black', s = 70, linewidths = 4, zorder = 7)
        ax.scatter(10**(-3.82), ypoint, marker='<',color='yellow', s = 20, linewidths = 4, zorder = 7)
...
name = '../data/multiNest/Rate-chains10k-test1/chains10k_test1_'


# +
a = pymultinest.Analyzer(outputfiles_basename = name, n_params = 3)
...
ax[0,0] = prof_likelihood(ax[0,0], whichy = 'Umu', whichx = 'm4', plot_name = 'BP1a')
ax[0,0].set_xlabel('')
