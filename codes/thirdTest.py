...
if len(int_prob_full) > 1:
    int_prob_pi_4     = np.mean(np.asarray(int_prob_full), axis = 0)
    int_prob_sup_pi_4 = np.mean(np.asarray(int_prob_sup_full), axis = 0)
    sigmas = np.mean(np.asarray(sigmas_full), axis = 0)
else:
    int_prob_pi_4 = int_prob
    int_prob_sup_pi_4 = int_prob_sup

rate_1sigma_pi_4 = np.ones(900) * -99
rate_2sigma_pi_4 = np.ones(900) * -99
rate_3sigma_pi_4 = np.ones(900) * -99

rate_1sigma_pi_4[np.where(sigmas[:,0] > cross_section_th)[0]] = 1
rate_2sigma_pi_4[np.where(sigmas[:,1] > cross_section_th)[0]] = 1
rate_3sigma_pi_4[np.where(sigmas[:,2] > cross_section_th)[0]] = 1
