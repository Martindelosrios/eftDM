...
class Network(swyft.SwyftModule):
    def __init__(self, lr = 1e-3, gamma = 1.):
        ...
        self.optimizer_init = swyft.OptimizerInit(torch.optim.Adam, dict(lr = lr, weight_decay=1e-5),
              torch.optim.lr_scheduler.ExponentialLR, dict(gamma = gamma))
        self.net = torch.nn.Sequential(
          torch.nn.Conv2d(1, 10, kernel_size=25),
          torch.nn.MaxPool2d(2),
          torch.nn.ReLU(),
          torch.nn.Dropout(0.2),
          torch.nn.Conv2d(10, 20, kernel_size=25),
          torch.nn.MaxPool2d(2),
          torch.nn.ReLU(),
          torch.nn.Dropout(0.2),
          torch.nn.Flatten(),
          torch.nn.Linear(720, 500),
          torch.nn.ReLU(),
          torch.nn.Dropout(0.2),
        ...
        marginals = ((0, 1), (0, 2), (1, 2))
        self.logratios1 = swyft.LogRatioEstimator_1dim(num_features = 60, num_params = 3, varnames = 'pars_norm')
        self.logratios2 = swyft.LogRatioEstimator_Ndim(num_features = 60, marginals = marginals, varnames = 'pars_norm')
...
checkpoint_callback     = ModelCheckpoint(monitor='val_loss', dirpath='./logs/', filename='O1_s1s2_NN2_{epoch}_{val_loss:.2f}_{train_loss:.2f}', mode='min')
...
if fit:
    ...
    checkpoint_callback.to_yaml("./logs/O1_s1s2_NN2.yaml") 
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_NN2.yaml")
else:
    ckpt_path = swyft.best_from_yaml("./logs/O1_s1s2_NN2.yaml")
...
if fit:
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('../graph/O1_loss_s1s2_NN2.pdf')
...
# +
# First let's create some observation from some "true" theta parameters
i = np.random.randint(ntest) # 189 (disc) 455 (exc) 203 (middle) 564
print(i)
...
10**(pars_true[1] * (pars_max[1] - pars_min[1]) + pars_min[1])
...
#ax[0,1].
#ax[1,0].grid(which = 'both')
#plt.savefig('../graph/2d_custom_posteriors_' + str(i) + '_s1s2_NN2.pdf')
...
if flag == 'exc':
    plt.savefig('../graph/O1_cornerplot_s1s2_exc_NN2.pdf')
else:
    plt.savefig('../graph/O1_cornerplot_s1s2_NN2.pdf')
...
if flag == 'exc':
    plt.savefig('../graph/1Dposterior_s1s2_exc_' + str(i) + '_NN2.pdf')
else:
    plt.savefig('../graph/1Dposterior_s1s2_disc_' + str(i) + '_NN2.pdf')
# -
...
if flag == 'exc':
    plt.savefig('../graph/loglikratio_s1s2_exc_NN2.pdf')
else:
    plt.savefig('../graph/loglikratio_s1s2_NN2.pdf')
# -
...
if flag == 'exc':
    plt.savefig('../graph/pars_s1s2_exc_NN2.pdf')
else:
    plt.savefig('../graph/pars_s1s2_NN2.pdf')
# -
# ### Let's make the contour plot
...
#itest = bps_ind[0]
    ...
    #ax[0,1].
    #ax[1,0].grid(which = 'both')
    plt.savefig('../graph/gif_plot_2Dposteriors_m84/' + str(c) + '_NN2.png')
# -
